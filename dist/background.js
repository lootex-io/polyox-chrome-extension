...
(() => {
  ...
  function createIcon(bitmap, size2, active) {
    const canvas = new OffscreenCanvas(size2, size2);
    const ctx = canvas.getContext("2d");
    if (!active) {
      ctx.globalAlpha = 0.4;
    }
    ctx.drawImage(bitmap, 0, 0, size2, size2);
    ctx.globalAlpha = 1;
    const dotRadius = Math.max(size2 * 0.15, 2);
    const dotX = size2 - dotRadius - 1;
    const dotY = size2 - dotRadius - 1;
    ctx.beginPath();
    ...
    ctx.fill();
    return ctx.getImageData(0, 0, size2, size2);
  }
  var cachedBitmap = null;
  async function setIconActive(active) {
    if (!cachedBitmap) {
      cachedBitmap = await loadIconBitmap();
    }
    const imageData = {};
    for (const size2 of [16, 32, 48, 128]) {
      imageData[size2] = createIcon(cachedBitmap, size2, active);
    }
    chrome.action.setIcon({ imageData });
  }

  // src/background.ts
  var API_BASE = "https://api-hoobs.polyox.io";
  ...
  async function redetectGame(tabId) {
    try {
  ...
  chrome.runtime.onMessage.addListener(
    (message, sender, sendResponse) => {
      handleMessage(message, sender).then((result) => sendResponse({ success: true, data: result })).catch(
  ...
  async function handleMessage(message, _sender) {
    ...
    switch (action) {
      ...
      case "detect": {
        ...
        const hasWallet = await executeInPage(
          tab.id,
      ...
      case "connect": {
        const tab = await getActiveTab();
        const accounts = await executeInPage(
          tab.id,
          () => window.ethereum.request({ method: "eth_requestAccounts" })
        );
        if (!accounts?.length) throw new Error("No accounts returned");
        const address = accounts[0];
        const chainIdHex = await executeInPage(
          tab.id,
      ...
      case "analyze": {
        ...
        try {
          ...
          await saveState({ analyzing: false, analysisError: message2 });
          throw err;
        }
      }
      ...
      case "get-history": {
        ...
        history.sort((a, b) => {
          const tA = a.analysis?.generatedAt ? new Date(a.analysis.generatedAt).getTime() : 0;
          const tB = b.analysis?.generatedAt ? new Date(b.analysis.generatedAt).getTime() : 0;
          return tB - tA;
        });
        return history;
      }
  ...
  async function performAnalysis(state) {
    ...
    const validAfter = 0;
    const validBefore = Math.floor(Date.now() / 1e3) + maxTimeout;
    ...
    await executeInPage(
      tab.id,
      (signer, data) => {
        const signerAddr = signer;
        const typedDataStr = data;
    ...
    const signature = await new Promise((resolve, reject) => {
      ...
      const poll = async () => {
        attempts++;
        try {
          const result = await executeInPage(tab.id, () => window.__polyoxSignResult);
          if (result) {
            if (result.error) {
